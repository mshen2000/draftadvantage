  ...
  var request = gapi.client.oauth2.userinfo.get().execute(function(resp) {
    if (!resp.code) {
	  ...
	  gapi.client.draftapp.main.authed().execute(
  	      function(resp) {
  	    	 if (resp.status = "OK") window.location.href = "index2.html";
 ...
 */
google.appengine.samples.hello.logon = function(
    email, password) {
  gapi.client.helloworld.greetings.customlogon({
      'email': email,
      'password': password
    }).execute(function(resp) {
      if (resp.status == "OK") {
...
google.appengine.samples.hello.enableButtons = function() {
  ...
  /*	
  var login_button = document.querySelector('#submit_button');
  login_button.addEventListener('click', function(e) {
    google.appengine.samples.hello.logon(
        document.querySelector('#email').value,
        document.querySelector('#password').value);
  });	
  
  
  var google_login_button = document.querySelector('#google_login_button');
  google_login_button.addEventListener('click', function(e) { 
	  
	  gapi.auth.authorize({client_id: google.appengine.samples.hello.CLIENT_ID,
	      scope: google.appengine.samples.hello.SCOPES, immediate: false},
	      google.appengine.samples.hello.userAuthed);
	  
    // google.appengine.samples.hello.auth();
  });	
  */
 ...
 * @param {string} apiRoot Root of the API's path.
 */
google.appengine.samples.hello.init_login = function(apiRoot) {
  // Loads the OAuth and helloworld APIs asynchronously, and triggers login
  // when they have completed.
  var apisToLoad;
  var callback = function() {
