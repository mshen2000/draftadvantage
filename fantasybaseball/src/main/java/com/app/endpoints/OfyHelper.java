...
public class OfyHelper implements ServletContextListener {
	...
	private static final Logger log = Logger
			.getLogger(OfyHelper.class.getName());
...
//		}
	...
	private boolean isAdminUserPresent() {
		...
		if (getIdentityService().isUserPresent("admin")) return Boolean.TRUE;
	...
	private boolean isGroupPresent(String groupId) {
		...
		if (getIdentityService().isRolePresent(groupId)) return true;
	...
	private void deleteTests(){	
		...
		if (getCustomTestService().isProgramPresent("internal")){
			...
			getCustomTestService().delete(getCustomTestService().get("internal").getId());
		}
		if (getCustomTestService().isProgramPresent("standard")){
			System.out.println("Found Standard Test, deleting it...");
			getCustomTestService().delete(getCustomTestService().get("standard").getId());
		}
...
//		fields.put("points", points);
		...
		CustomObjectTest test = new CustomObjectTest("internal", "Internal Tests");
...
//		fields.put("points", points);
	...
	private void createPrograms(){
		...
		BaseFieldProgramScore addition_score = new BaseFieldProgramScore("addition_score");
		...
		BaseFieldFloat addition_scorepercent = new BaseFieldFloat("addition_scorepercent");
		...
		BaseFieldProgramScore subtraction_score = new BaseFieldProgramScore("subtraction_score");
		...
		BaseFieldFloat subtraction_scorepercent = new BaseFieldFloat("subtraction_scorepercent");
		...
		BaseFieldProgramScore multiplication_score = new BaseFieldProgramScore("multiplication_score");
		...
		BaseFieldFloat multiplication_scorepercent = new BaseFieldFloat("multiplication_scorepercent");
		multiplication_scorepercent.setQuery_alias("Multiplication Score Percent");
		multiplication_scorepercent.setForm_alias("Multiplication Score Percent");
		...
		BaseFieldProgramScore division_score = new BaseFieldProgramScore("division_score");
		...
		BaseFieldFloat division_scorepercent = new BaseFieldFloat("division_scorepercent");
...
//		System.out.println("group 1 ID: " + groups.get(0).getId());
		...
		for (Role role : roles){
			...
			if (role.getUsers().size() > 0){
				...
				List<User> roleusers = getIdentityService().getRoleUsers(role.getName());
		...
		for (User user : users){
			...
			System.out.println("# of roles: " + getIdentityService().getUserRoles(user.getUsername()).size());
